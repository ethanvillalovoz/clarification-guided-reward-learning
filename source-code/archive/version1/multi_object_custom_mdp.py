...
class Gridworld():
    ...
    def create_initial_state(self):
        ...
        for obj in self.initial_object_locs:
            state = {}
            state['grid'] = copy.deepcopy({obj: self.initial_object_locs[obj]}) # type tuple (color, type) to location
            state['exit'] = False
    ...
    def render(self, current_state, timestep):
        ...
        if all(state['exit'] for state in current_state):
        ...
        for i in range(0, len(plot_init_state)):
            orientation = plot_init_state[i]['orientation']
            for type_o in plot_init_state[i]['grid']:
                loc = plot_init_state[i]['grid'][type_o]
        ...
        for i in range(len(states)):
            ...
            for action_idx_i in range(len(actions)):
                ...
                if isinstance(team_reward, (np.ndarray, list)):
                    reward_mat[i, action_idx_i] = team_reward[0]  # or any other indexing as required
                else:
    ...
    def rollout_full_game_joint_optimal(self):
        ...
        for i in range(0, len(self.current_state)):
            while not done:
                ...
                self.render(self.current_state[i], iters)
...
def compute_new_policy(reward_weights, true_f_idx, object_type_tuple, red_centroid, blue_centroid):
    ...
    return game.policy, game.state_to_idx, game.idx_to_state, game.idx_to_action, game.action_to_idx, game.transitions, game.rewards
...
if __name__ == '__main__':
    ...
    reward_weights = [[-10, -2, -2, -2],
                      [-2, 1, -1, 1]]  # orientation, red prox, blue prox, pos y
